...
synchronize frames to a clock, looks like this:
    ...
    1) read output of CmdlineGL until "t=" event indicating current timestamp
    2) clear frame buffer
    3) render OpenGL graphics
    4) call glFlush and cglSwapBuffers to show the rendering
    5) call cglSync telling it to delay until T + framerate_period
    6) call cglGetTime, so that new timestamp will be available immediately after sleep
    7) repeat
